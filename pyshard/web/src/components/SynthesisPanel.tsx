...
import { Sparkles, Loader2, AlertTriangle, CheckCircle, FileCode, ChevronDown, ChevronUp, Package, Smartphone } from 'lucide-react'
import { Shard, SynthesisResult } from '../types'

type SynthesisMode = 'python' | 'web-to-app'

export function SynthesisPanel({ selectedShards, onSynthesized, mode }: { selectedShards: Shard[]; onSynthesized: (result: any) => void; mode: SynthesisMode }) {
  ...
  const handleSynthesize = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'python') {
        ...
        setStep('Generating WebToApp module...')
        await new Promise(r => setTimeout(r, 500))
        const res = await fetch('/api/wta/generate-module', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            shard: selectedShards[0],
            output_dir: `.pyshard_output/wta_modules/${projectName.trim() || 'module'}`,
          }),
        })
        if (!res.ok) throw new Error((await res.json()).detail || 'Module generation failed')
        const data = await res.json()
        setResult(data)
        onSynthesized(data)
      }
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
      <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
        {mode === 'python' ? (
          ...
          <><Smartphone className="w-5 h-5 text-emerald-400" /> Step 3: Generate WebToApp Module</>
        )}
      </h2>
      ...
      <form onSubmit={handleSynthesize} className="space-y-4">
        <div>
          <label className="block text-xs font-medium text-slate-400 mb-1">
            {mode === 'python' ? 'Project Name' : 'Module Name'}
          </label>
          <input
            value={projectName}
        ...
        <div>
          <label className="block text-xs font-medium text-slate-400 mb-1">
            {mode === 'python' ? 'App Description' : 'Module Description'}
          </label>
          <textarea
            value={description}
            ...
            placeholder={
              mode === 'python'
                ? 'Describe the app you want to build from these shards...'
                : 'Describe what this WebToApp module should do...'
            }
            rows={4}
        ...
        >
          {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> {step || (mode === 'python' ? 'Synthesizing...' : 'Generating Module...')}</> : (mode === 'python' ? 'Generate Project' : 'Generate Module')}
        </button>
      </form>
      ...
      {result && (
        <div className="mt-6 space-y-4">
          <div className="flex items-center gap-2 text-sm text-emerald-400">
            ...
            {mode === 'python' ? (
              <>Project generated at: <code className="text-xs bg-slate-950 px-2 py-1 rounded">{result.output_path}</code></>
            ) : (
              <>Module generated: <code className="text-xs bg-slate-950 px-2 py-1 rounded">{result.module_dir}</code></>
            )}
          </div>

          {mode === 'python' && result.gaps?.length > 0 && (
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-300 mb-2">
              {mode === 'python' ? 'Generated Files' : 'Module Files'}
            </h3>
            <div className="space-y-1">
              {(mode === 'python' ? result.files : [
                { path: result.module_dir, purpose: 'Module directory' },
                { path: result.wtamod, purpose: 'WebToApp module package' },
                { path: result.module_json, purpose: 'Module manifest' },
                { path: result.main_js, purpose: 'JavaScript content script' },
              ]).map((f: any, i: number) => (
                <div key={i} className="border border-slate-800 rounded-lg overflow-hidden">
                  ...
                  {expandedFiles.has(f.path) && (
                    <pre className="p-3 bg-slate-950 text-xs text-slate-400 overflow-x-auto max-h-64 font-mono border-t border-slate-800">
                      {f.content || '[binary or generated file]'}
                    </pre>
                  )}
                </div>
              ))}
